...
export class PurgeCSSIntegration {
  ...
  defaultExtractor(content) {
    ...
    // Class selectors
    const classMatches = content.match(/[\s"'`][a-zA-Z][\w-]*[^\s"'`]/g) || [];
    selectors.push(
      ...classMatches.map((match) => match.trim().replace(/['"`]/g, "")),
    );

    // ID selectors
    const idMatches = content.match(/#[a-zA-Z][\w-]*/g) || [];
    selectors.push(...idMatches);
  ...
  parseCSSSelectors(css) {
    ...
    // This is a simplified approach - in production, you'd use a proper CSS parser
    ...
    mediaMatches.forEach((match) => {
      const innerCSS = match.replace(/@media[^{]*\{/, "").replace(/\}$/, "");
      const innerSelectors = innerCSS.match(/([^{]+)\s*\{/g) || [];
      innerSelectors.forEach((innerMatch) => {
        const selector = innerMatch.replace(/\s*\{.*$/, "").trim();
        if (selector) {
          selectors.add(selector);
        }
      });
  ...
  async removeUnusedCSS(css, usedSelectors) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmedLine.includes('{') && !trimmedLine.startsWith('@') && braceCount === 1) {
    ...
    for (const rule of rules) {
      if (rule.selector === 'non-rule') {
        ...
        const isUsed = usedSet.has(rule.selector) || 
                      usedSelectors.some(used => rule.selector.includes(used)) ||
    ...
    if (this.options.rejected && rejectedSelectors.length > 0) {
      ...
      if (rejectedSelectors.length > 10) {
        console.log(chalk.gray(`   ... and ${rejectedSelectors.length - 10} more`));
      }
    }
    ...
    return {
      ...
      stats: {
        ...
        compressionRatio: css.length > 0 ? ((css.length - optimizedCSS.length) / css.length) * 100 : 0
      },
      rejectedSelectors
    };
  }
        ...
        // End of current rule
        currentRule.push(line);
        selectorToRule.set(currentSelector, {
          rule: currentRule.join("\n"),
          lineNumber: i - currentRule.length + 1,
        });
        ...
        // Outside of rules (comments, @import, etc.)
        if (currentSelector && currentRule.length > 0) {
          selectorToRule.set(currentSelector, {
            rule: currentRule.join("\n"),
            lineNumber: i - currentRule.length,
          });
        }
        currentSelector = "";
        currentRule = [];
    ...
    for (const [selector, ruleInfo] of selectorToRule) {
      const isUsed =
        usedSet.has(selector) ||
        usedSelectors.some((used) => selector.includes(used)) ||
        this.shouldPreserveSelector(selector);

      if (isUsed) {
        preservedRules.push(ruleInfo.rule);
        preservedCount++;
      } else {
        removedRules.push(ruleInfo.rule);
        rejectedSelectors.push(selector);
        removedCount++;
      }
    }

    // Rebuild CSS with preserved rules
    let optimizedCSS = preservedRules.join("\n\n");

    // Add back any non-rule content (comments, @import, etc.)
    const nonRuleContent = lines
      .filter(
        (line) =>
          !line.includes("{") &&
          !line.includes("}") &&
